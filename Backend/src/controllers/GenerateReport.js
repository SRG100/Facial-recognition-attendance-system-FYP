...
router.get('/generateReport', async (req, res) => {
    try {
        let { Module, From, To, Id, UserRole, Teacher_Id } = req.query;
        console.log("Report request params:", Module, From, To, Id, UserRole, Teacher_Id);
        ...
        try {
            ...
            if (UserRole === "teacher") {
                try {
                    const [Modules] = await db.query(
                        `SELECT module_id FROM teacher_association WHERE teacher_id = ?`, 
                        [Teacher_Id]
                    );
                    
                    if (Modules && Modules.length > 0) {
                        Module = Modules[0].module_id;
                        console.log("Teacher's module found:", Module);
                    } else {
                        console.log("No modules found for teacher:", Teacher_Id);
                        return res.status(404).json({ 
                            success: false, 
                            message: "No module found for this teacher." 
                        });
                    }
                } catch (err) {
                    console.error("Error in teacher module query:", err);
                    return res.status(500).json({ 
                        success: false, 
                        message: "Error retrieving teacher's module information" 
                    });
                }
            }
            
            console.log("Building attendance query with params:", Module, From, To, Id);
            let query = `SELECT s.Student_id, s.Student_name, aa.Section_id, m.Module_Name, m.Module_id, 
                          c.Class_id, c.Class_Type, 
            ...
            try {
                ...
                console.error("Error executing attendance query:", err);
                return res.status(500).json({ 
                    success: false, 
            ...
            if (allRecords.length === 0) {
                console.log("No records found for criteria");
                return res.status(404).json({ 
                    success: false, 
            ...
            try {
                ...
                for (const moduleId in moduleGroups) {
                    ...
                    // Make sure we have records before accessing them
                    ...
                    // Student information - corrected order
                    doc.fontSize(14).text(`Student Name: ${Student_name}`, { align: "center" });
                    doc.moveDown();
                    doc.fontSize(14).text(`Student ID: ${Student_id}`, { align: "center" });
                    doc.moveDown();
                    ...
                    // Draw header - make sure this function exists
                    try {
                        drawTableHeader(doc, startX, currentY, colWidths, rowHeight);
                        currentY += rowHeight;
                    } catch (err) {
                        console.error("Error in drawTableHeader function:", err);
                        throw new Error("drawTableHeader function failed");
                    }
                    ...
                    records.forEach((record, index) => {
                        ...
                        if (currentY + rowHeight > doc.page.height - 50) {
                            ...
                            try {
                                ...
                                console.error("Error in drawTableHeader function on new page:", err);
                                throw new Error("drawTableHeader function failed on new page");
                            }
                        }
                        
                        // Draw row - make sure this function exists
                        try {
                            ...
                            console.error("Error in drawTableRow function:", err);
                            throw new Error("drawTableRow function failed");
                        }
                    });
                }
